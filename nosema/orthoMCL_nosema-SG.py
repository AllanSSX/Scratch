...
def main(args):
	...
	groups = {}
	Nosema = ['NGRA', 'NANT', 'NAPI', 'NBOM', 'NCEP', 'NYNP']
	...
	for line in args.group:
		...
		groups[cluster] = {'seq':seqs, 'species':{}, 'nosema_sp':{}}
		for gene in seqs:
			specie, id = gene.split('|')
			if specie not in groups[cluster]['species']:
				groups[cluster]['species'][specie] = 1
				if specie in Nosema and specie not in groups[cluster]['nosema_sp']:
					groups[cluster]['nosema_sp'][specie] = 1
	...
	for cluster, infos in sorted(groups.items()):
		if len(infos['nosema_sp']) == 6 and len(infos['species']) == 6 and len(infos['seq']) == 6:
			# print(cluster, infos)
